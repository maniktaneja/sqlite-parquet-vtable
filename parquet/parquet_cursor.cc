...
bool ParquetCursor::currentRowGroupSatisfiesBlobFilter(Constraint& constraint, std::shared_ptr<parquet::Statistics> _stats) {
  ...
  if(pqType == parquet::Type::BYTE_ARRAY) {
    parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::BYTE_ARRAY>>* stats =
      (parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::BYTE_ARRAY>>*)_stats.get();
...
bool ParquetCursor::currentRowGroupSatisfiesIntegerFilter(Constraint& constraint, std::shared_ptr<parquet::Statistics> _stats) {
  ...
  if(pqType == parquet::Type::INT32) {
    parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::INT32>>* stats =
      (parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::INT32>>*)_stats.get();

    min = stats->min();
    max = stats->max();
  } else if(pqType == parquet::Type::INT64) {
    parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::INT64>>* stats =
      (parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::INT64>>*)_stats.get();

    min = stats->min();
    max = stats->max();
  } else if(pqType == parquet::Type::INT96) {
    parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::INT96>>* stats =
      (parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::INT96>>*)_stats.get();

    min = int96toMsSinceEpoch(stats->min());
    max = int96toMsSinceEpoch(stats->max());

  } else if(pqType == parquet::Type::BOOLEAN) {
    parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::BOOLEAN>>* stats =
      (parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::BOOLEAN>>*)_stats.get();
...
bool ParquetCursor::currentRowGroupSatisfiesDoubleFilter(Constraint& constraint, std::shared_ptr<parquet::Statistics> _stats) {
  ...
  if(pqType == parquet::Type::DOUBLE) {
    parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::DOUBLE>>* stats =
      (parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::DOUBLE>>*)_stats.get();

    min = stats->min();
    max = stats->max();
  } else if(pqType == parquet::Type::FLOAT) {
    parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::FLOAT>>* stats =
      (parquet::TypedStatistics<parquet::PhysicalType<parquet::Type::FLOAT>>*)_stats.get();
    ...
    if(column == -1) {
      ...
      if(md->is_stats_set()) {
        ...
        if(op == IsNull) {
          ...
          parquet::Type::type pqType = types[column];
          // Changed from UTF8 to STRING - Manik
          if(pqType == parquet::Type::BYTE_ARRAY && logicalTypes[column] == parquet::LogicalType::Type::STRING) {
...
start:
  ...
  while(logicalTypes.size() < (unsigned int)rowGroupMetadata->num_columns()) {
   logicalTypes.push_back(rowGroupMetadata->schema()->Column(0)->logical_type()->type());
  }

  for(unsigned int i = 0; i < (unsigned int)rowGroupMetadata->num_columns(); i++) {
    types[i] = rowGroupMetadata->schema()->Column(i)->physical_type();
    logicalTypes[i] = rowGroupMetadata->schema()->Column(i)->logical_type()->type();
  }
...
bool ParquetCursor::currentRowSatisfiesFilter() {
  ...
  for(unsigned int i = 0; i < constraints.size(); i++) {
    ...
    if(op == IsNull) {
      ...
      // Changed from UTF8 to String - Manik
      if(logicalTypes[column] == parquet::LogicalType::Type::STRING) {
...
parquet::LogicalType::Type::type ParquetCursor::getLogicalType(int col) {
